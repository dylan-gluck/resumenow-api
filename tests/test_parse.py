...
def test_parse_resume_pdf_success(mock_client):
    ...
    assert "Python" in [skill["name"] for skill in response_data["data"]["technical_skills"]["programming_languages"]]
...
def test_parse_resume_text_success(mock_client):
    ...
    test_file = io.BytesIO(b"Jane Smith\nSenior Developer\nSkills: Java, Spring, Kubernetes")
    
    # Make the request
    response = client.put(
        "/parse",
        files={"file": ("resume.txt", test_file, "text/plain")}
    )
    ...
    assert "Java" in [skill["name"] for skill in response_data["data"]["technical_skills"]["programming_languages"]]
...
def test_parse_resume_markdown_success(mock_client):
    ...
    test_file = io.BytesIO(b"# Alex Johnson\n\n## Skills\n- React\n- Node.js\n- MongoDB")
    
    # Make the request
    response = client.put(
        "/parse",
        files={"file": ("resume.md", test_file, "text/markdown")}
    )
    ...
    assert "React" in [skill["name"] for skill in response_data["data"]["technical_skills"]["frameworks_libraries"]]
...
def test_parse_resume_unsupported_file_type():
    ...
    response = client.put(
        "/parse",
        files={"file": ("resume.html", test_file, "text/html")}
    )
